			...
			if (i == 1) {
				...
				for (int alpha : alphas) {
					for (int beta : betas) {
						try (PrintWriter logger = new PrintWriter(
								new BufferedWriter(new FileWriter("result_919s.txt", true)))) {
							evaluateFromTXTQuery(0, 1, "./finaltest_new.txt", alpha, 0, logger, i, 1, beta, 2, true,
									resultDB);
						} catch (IOException e) {
							e.printStackTrace();
						}
				...
				for (int trainVal : trainVals)
					for (int alpha : alphas) {
						for (int beta : betas) {
							try (PrintWriter logger = new PrintWriter(
									new BufferedWriter(new FileWriter("result_919s.txt", true)))) {
								evaluateFromTXTQuery(0, 1, "./finaltest_new.txt", alpha, 0, logger, i, trainVal, beta,
										2, true, resultDB);
							} catch (IOException e) {
								e.printStackTrace();
							}
				...
				for (int trainVal : trvs) {
					for (float gama : gamas) {
						try (PrintWriter logger = new PrintWriter(
								new BufferedWriter(new FileWriter("result_919s.txt", true)))) {
							evaluateFromTXTQuery(gama, 1, "./finaltest_new.txt", alpha, 0, logger, i, trainVal, beta, 2,
									true, resultDB);
						} catch (IOException e) {
							e.printStackTrace();
						}
		...
		for (float gama : gamas) {
			for (int trainVal : trainVals) {
				try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter(printFile, true)))) {
					evaluateFromTXTQuery(gama, 1, path, alpha, 0, logger, 4, trainVal, beta, 2, true, resultDB);
				} catch (IOException e) {
					e.printStackTrace();
				}
			...
			if (i == 1) {
				...
				for (int alpha : alphas) {
					for (int beta : betas) {
						if (alpha > beta) {
							try (PrintWriter logger = new PrintWriter(
									new BufferedWriter(new FileWriter("result_919.txt", true)))) {
								evaluateFromTXTQuery(0, 1, path, alpha, 0, logger, i, 1, beta, 2, true, resultDB);
							} catch (IOException e) {
								e.printStackTrace();
							}
						}
...
//                                } else {
								evaluateFromTXTQuery(0, 1, path, alpha, 0, logger, i, trainVal, beta, 2, true,
										resultDB);
							} catch (IOException e) {
								e.printStackTrace();
							}
				...
				for (int trainVal : trvs) {
					for (float gama : gamas) {
						try (PrintWriter logger = new PrintWriter(
								new BufferedWriter(new FileWriter("result_919.txt", true)))) {
							evaluateFromTXTQuery(gama, 1, path, alpha, 0, logger, i, trainVal, beta, 2, true, resultDB);
						} catch (IOException e) {
							e.printStackTrace();
						}
					}
		...
		for (int alpha : alphas) {
			for (int beta : betas) {
				try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter("result_919.txt", true)))) {
					evaluateFromTXTQuery(0, 1, "./finaltest_new.txt", alpha, 0, logger, 1, 1, beta, 2, true, resultDB);
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	static void extraExp2() { // For trainMode 2
		...
		for (int v : tvs) {
			try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter("result_919.txt", true)))) {
				evaluateFromTXTQuery(0, 1, "./finaltest_new.txt", alpha, 0, logger, 2, v, beta, 2, true, resultDB);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	static void extraExp3() { // For trainMode 3
		...
		for (int trainVal : trainVals) {
			try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter("result_919.txt", true)))) {
				evaluateFromTXTQuery(0, 1, "./finaltest_new.txt", alpha, 0, logger, 3, trainVal, beta, 2, true,
						resultDB);
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	static void baselineEvaluation() {
		try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter("result_new.txt", true)))) {
			evaluateFromTXTQuery(0, 2, "./test_file/sensys_truth_v1.txt", 0, 0, logger, 0, 0, 0, 2, false,
					"result_0331");
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	static void evaluateFromTXTQuery(float gama, int evalMode, String path, int alpha, int verbose, PrintWriter logger,
			int trainMode, int trainModeVal, int beta, int probMode, boolean dbMode, String resultDB) {
		...
		try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(path)))) {
			...
			while ((name = br.readLine()) != null) {
				...
				if (evalMode == 1) {
					result = LocationService.queryTimestamp(name, timeStamp, alpha, trainMode, trainModeVal, gama, beta,
							probMode);
				} else if (evalMode == 2) {
			...
			logger.printf("Alpha: %d, TrainMode: %d, TrainModeVal: %d, Beta: %d, Gama: %.2f, ProbM: %d %n", alpha,
					trainMode, trainModeVal, beta, gama, probMode);
			...
			logger.printf("Correct: %d, Total: %d, Prediction Accuracy: %.2f. %n", (correctOut + correctAP), total,
					acc);
			...
			if (dbMode) {
				try (Connect connect = new Connect("local")) {
					...
					if (!LocationService.checkLocalTableExist(resultDB)) {
						...
						s.execute("create table " + resultDB
								+ " (expertime datetime, tesefile varchar(255), alpha integer, beta integer, trainMode integer, trainVal integer, probMode integer, "
...
/*
 * class APtoRoom implements Serializable { List<String> aps; Map<String,
 * List<String>> apMapRoom; static final String path = "ap2room.dat";
 * 
 * void init() { aps = new ArrayList<>(); apMapRoom = new HashMap<>(); try
 * (Connect connect = new Connect("server")) { Connection connection =
 * connect.getConnection(); Statement st1 = connection.createStatement();
 * ResultSet rs1 =
 * st1.executeQuery("select id from SENSOR where sensor_type_id = 1;"); while
 * (rs1.next()) { aps.add(rs1.getString(1)); } PreparedStatement ps1 =
 * connection.
 * prepareStatement("select INFRASTRUCTURE.name from SENSOR, COVERAGE_INFRASTRUCTURE, INFRASTRUCTURE\n"
 * +
 ...
 * ); for (String ap : aps) { ps1.setString(1, ap); ResultSet rs2 =
 * ps1.executeQuery(); List<String> rooms = new ArrayList<>(); while
 * (rs2.next()) { rooms.add(rs2.getString(1)); } apMapRoom.put(ap, rooms); } }
 * catch (SQLException e) { e.printStackTrace(); }
 * 
 * try (ObjectOutputStream oos = new ObjectOutputStream(new
 * FileOutputStream(path))) { oos.writeObject(apMapRoom);
 * System.out.println("Successfully write to disk."); } catch (IOException e) {
 * e.printStackTrace(); }
 * 
 * }
 * 
 * void load() { try (ObjectInputStream ooi = new ObjectInputStream(new
 * FileInputStream(path))) { apMapRoom = (Map<String, List<String>>)
 * ooi.readObject(); } catch (IOException e) { e.printStackTrace(); } catch
 * (ClassNotFoundException e) { e.printStackTrace(); } }
 * 
 * void print() { System.out.println("AP number: " + apMapRoom.size()); }
 * 
 * List<String> find(String ap) { return apMapRoom.get(ap); } }
 */
/*
 * class RoomToAP { Map<String, List<String>> room2ap;
 * 
 * RoomToAP(APtoRoom aptoRoom) { room2ap = new HashMap<>(); for
 * (Map.Entry<String, List<String>> entry : aptoRoom.apMapRoom.entrySet()) { for
 * (String room : entry.getValue()) { List<String> roomList = new
 * ArrayList<String>(); //room2ap.getOrDefault(room, new ArrayList<>()); ihe
 * roomList.add(entry.getKey()); room2ap.put(room, roomList); } } }
 * 
 * void print() { System.out.println("Total Room: " + room2ap.size()); }
 * 
 * List<String> find(String room) { return room2ap.get(room); }
 * 
 * Set<String> allAPsOfRoom(String room) { Set<String> aps = new HashSet<>();
 * for (String ap:find(room)){ aps.add(ap); } return aps; } }
 */
class NameToMac {
