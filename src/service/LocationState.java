...
public class LocationState {
	...
	public void print() {
		System.out.println(String.format("AP: %s, Inside Classification: %s, Possible Rooms: %s", accessPoint, inside,
				possibleRooms.toString()));
	}
	...
	boolean verifyIn(String truth, int verbose, String timestamp, String name) {
		if (!inside) {
			if (verbose >= 2) {
				System.out.println(
						String.format("%s: %s Predicted outside, truth is inside, room: %s.", name, timestamp, truth));
			}
			return false;
		}
		return true;
	...
	boolean verifyAP(String truth, APtoRoom aptoRoom, int verbose, String timestamp, String name) {
		...
		if (verbose >= 1) {
			System.out.println(String.format("%s: %s Predicted AP: %s, its room list %s, true room: %s", name,
					timestamp, accessPoint, possibleRoom.toString(), truth));
		}
		return false;
	}
