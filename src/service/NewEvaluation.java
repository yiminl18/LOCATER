...
public class NewEvaluation {
	...
	static void TestWithFixedParameters() {
		...
		try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter(logPath, true)))) {
			evaluateFromTXTQuery(path, 20, 100, "20180101", "20180630", 4, 2, logger, true, resultDB);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
			...
			for (int pos : pos_list)
				for (int neg : neg_list)
					for (int step : step_list) {
						evaluateFromTXTQuery(path, pos, neg, "20180101", "20180630", step, 2, logger, true, resultDB);
					}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	static void TestWithDifferentAmoutData() {
		...
		int pos = 25;
		int neg = 200;
		String[] start_list = { "20180510", "20180501", "20180420", "20180410", "20180401", "20180310", "20180225",
				"20180201" };
		try (PrintWriter logger = new PrintWriter(new BufferedWriter(new FileWriter(logPath, true)))) {
			for (String start : start_list) {
				evaluateFromTXTQuery(path, pos, neg, start, "20180630", 2, 2, logger, true, resultDB);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	static void evaluateFromTXTQuery(String path, int pos, int neg, String start_day, String end_day, int step,
		...
		try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(path)))) {
			...
			logger.printf(
					"Positive threshold: %d, Negative threshold: %d, Training data: %s ~ %s, training step: %d. %n",
